...
def read_h5_files(path, files, print_info=False):
    ...
    for f in files:
        ...
        pd_data['x'] = np.array(x)
        pd_data['y'] = np.array(y)
        pd_data['id'] = np.array(video_id_list)
        pd_data.set_index()
...
def gen_data():
    ...
    audioset_data = read_h5_files(path, files, print_info=False)
    ...
    dataset = pd.DataFrame() # will contain strongly labeled data
    #dataset.append(dcase_eval.iloc[0:10])

    for ka in audioset_data.keys():
        v = audioset_data[k][params[2]]
        find_id = '9OqtuFGCCR8'#'Sb0169-lqLs'
        #print(v.)
        for i in v:
            #print(str(i)[2:-1])
            if str(i)[2:-1] == find_id:
                print(str(i)[2:-1],' == '+find_id)
                break
