...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        mult = norm_att * cla
        x = torch.sum(mult, dim=2)

        return x, cla, norm_att, mult
...
class DecisionLevelSingleAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        output, cla, norm_att, mult = self.attention(b1)
...
class DecisionLevelMultiAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        # (samples_num, classes_num)
        output1, cla, norm_att, mult = self.attention(emb_layers[-1])
        output2, cla2, norm_att2, mult2 = self.attention(emb_layers[-2])
        ...
        return output, cla, cla2, norm_att, norm_att2, mult, mult2
...
if __name__ == '__main__':
    ...
    if totest == 0 and (not (args.mode == 'get_avg_stats')): 
        ...
        if args.mode == "train":
            ...
            args.bgn_iteration = 200
            args.fin_iteration = 2601
            args.interval_iteration = 200
